...
void debug_http_obj(http_t *http_obj)
{
    ...
    printf("=============Start-Line=============\n");
    printf("[%s][%s]\n", get_http_version_by_idx[http_obj->version], http_obj->type==REQ? "REQUEST": "RESPONSE");
    printf("[Connect to %s:%d]\n", http_obj->target, http_obj->port);
    if(http_obj->type==REQ){
        printf("[Method: %s][Request Target: %s]\n", get_http_method_token_by_idx[http_obj->req.method_token], http_obj->req.req_target);
    } else {
        printf("[Status code: %s][Reason Phrase: %s]\n", get_http_status_code_by_idx[http_obj->req.method_token], http_obj->req.req_target);
    }
